 ...
 * @returns    { boolean }
 */
export function iSSquareAndNumc(arr) {
    let lng = arr.length;
    let msgW = 'Something are wrong square matrix';
    let ind = 0;
    try {
 ...
 * @returns
 */
export function iSNumVect(vect) {
    try {
        vect.forEach((num) => {
            if (isNaN(num))
                throw (new DataError(`Error in vector this have a NOT Number [ ${num} ]`));
        });
    }
    catch (error) {
...
export function iSNumeric(number) {
    try {
        if (isNaN(number))
            throw (new DataError(`Error value is NOT a Number [ ${number} ]`));
    }
    catch (error) {
        return result(false, error, NaN);
    }
...
export function iSNumcMtrxAndHasNVec(arr, vectrsNum) {
    try {
        if (!(vectrsNum == arr.length))
            throw (new DataError(`Matrix have not number rows required = required: ${vectrsNum} | gived ${arr.length} `));
        if (arr.length > 1) {
            arr.forEach((vector) => {
                vector.forEach((number) => {
                    if (isNaN(number))
                        throw (new DataError(`Error matrix has a NOT number [ ${number} ]`));
                });
            });
        }
        else {
...
export function hasAzeroOrNaN(vector) {
    try {
        vector.forEach((number) => {
            if (number == 0 || isNaN(number))
                throw (new DataError(`Error at the value, this a NaN or Zero - value: ${number} `));
        });
    }
    catch (error) {
        return result(false, error, NaN);
