 ...
 */
export function determinant2x2(arr) {
    // arr = [
    ...
    if (determinant == 0)
        return result(true, `¡warning determinant = 0! data: [ ${arr[0]}], [${arr[1]}] ]`, determinant);
 ...
 * @returns determinant { number }
 */
export function determinant3x3(arr_input) {
    let leftToRight = 0;
    let RightToleft = 0;
    let delta = 0;
    let cont = 0;
    ...
    if (delta == 0)
        return result(true, `¡warning determinant = 0! data: [ ${arr[0]}], [${arr[1]}], [${arr[2]}]`, delta.toString());
 ...
 * @returns   { number }
 */
export function determinant4x4(arr) {
    let delta = 0;
    let result4x4;
    // Block of validations
    let { status, msg, res } = iSSquareAndNumc(arr);
    if (!status)
        return result(status, msg + ' [determinant4x4]', res);
    // end block
    // [ a, b, c, d ]
    // [ f, g, h, i ]
    // [ k, l, m, n ]
    ...
    if (delta == 0)
        return result(true, `¡warning determinant = 0! data: [ ${arr}`, delta);
    return result(true, '', delta);
}
/**
 ...
 */
function setArray4x4(arr, range, skipt) {
    let vectTmp = [];
    let vectSnd = [];
    for (let i = 1; i < 4; i++) {
