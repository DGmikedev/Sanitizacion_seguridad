 ...
 * @returns    { array inv }
 */
export function getInvArr2x2(arr, det) {
    let invA = [];
    // Block of validations
    let { status, msg, res } = iSSquareAndNumc(arr);
    if (!status)
        return result(status, msg + ' [getInvArr2x2[x][]]', res);
    if (det === 0)
        return result(false, `Determinant is equal to zero [getInvArr2x2[][${det}]]`, NaN);
    let resn = iSNumeric(det);
    if (!resn.status)
        return result(resn.status, resn.msg + ' [getInvArr2x2[][x]]', resn.res);
    // end block    
    try {
 ...
 */
export function isInvers2x2(arr, isInvrs) {
    let identityMtrx;
    let acm = 0;
    // Block of validations
    let arrO = iSSquareAndNumc(arr);
    if (!arrO.status)
        return result(arrO.status, arrO.msg + ' [isInvers2x2[x][]]', arrO.res);
    let arrInv = iSSquareAndNumc(isInvrs);
    if (!arrInv.status)
        return result(arrInv.status, arrInv.msg + ' [isInvers2x2[][x]]', arrInv.res);
    // end block 
    try {
        identityMtrx = [
        ...
        if (acm != 2)
            return result(false, 'Array 2 Not inverse of Array1', false);
    }
    catch (error) {
        return result(false, error, NaN);
    }
