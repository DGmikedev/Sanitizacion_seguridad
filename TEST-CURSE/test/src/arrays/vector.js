 ...
 * @returns number
 */
export function vctrSum(vector) {
    // handle the result
    let sum = 0;
    let { status, msg, res } = iSNumVect(vector);
    if (!status)
        return result(status, msg + ' [vctrSum]', res);
    try {
 ...
 * @returns       { vector * mult }
 */
export function vctrXn(vector, mult) {
    let vectRes = [];
    let check;
    // Block of validations
    check = iSNumeric(mult);
    if (!check.status)
        return result(check.status, check.msg + ' [vctXn([][x])]', check.res);
