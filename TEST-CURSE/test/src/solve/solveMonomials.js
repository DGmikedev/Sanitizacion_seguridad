 ...
 * @returns
        ...
        3.- return => 48
 */
export function evalMonomial(coefficient, exponent, valueVaraible) {
    let res = 0;
    let errScan = iSNumVect([coefficient, exponent, valueVaraible]);
    if (!errScan.status)
        return result(false, errScan.msg, NaN);
    try {
        res += (coefficient) * (Math.pow(valueVaraible, (exponent)));
    }
    catch (error) {
