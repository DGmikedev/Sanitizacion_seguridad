 ...
 *
 */
export function solveLn2x2(arr) {
    let x = 0; // x value
    let y = 0; // y value
    let t = 0; // degree of testing  0 and 1 = bad, 2 = ok
    let delta = 0;
    let deltaX = 0;
    let deltaY = 0;
    let check = iSNumcMtrxAndHasNVec(arr, 2);
    // Block of validation
    if (!check.status)
        return result(check.status, check.msg + ' [solveLn2x2]', check.res);
    // [a, b, c]
    // [d, e, f]
    try {
        // delta         [a, b] [d, e]          
        ...
        if (delta == 0)
            return result(false, 'delta = 0, infity solutions betweet equations', { x: NaN, y: NaN });
        // testing values in eq1
        if (((arr[0][0] * x) + (arr[0][1] * y)) == arr[0][2])
            t++;
        // testing values in eq2
        if (((arr[1][0] * x) + (arr[1][1] * y)) == arr[1][2])
            t++;
    }
    catch (error) {
