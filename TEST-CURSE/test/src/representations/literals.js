 ...
 * @returns
 */
export function getRepresentEquationPolinomial(values, degrees, variable = 'x') {
    let eq = '';
    let value = '';
    let scanErr;
    // Block of validation
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        if (i != values.length - 1) {
            ...
            if (value === '-1')
                value = value.slice(0, 1); // set just '-'
            if (value === '+1')
                value = value.slice(0, 1); // set just '+'
        }
 ...
 * @returns
 */
function makeMonomial(value, variable, degree) {
    let mono = '';
 ...
 */
export function setRprstDerivEq(arrVal, arrDegre, variable = 'x') {
    let eq = '';
    let value = '';
    let degre = '';
    let tmp = '';
